...
function toCamelCase(name, uppercase) {
  let res = '';
  name.split('-').forEach((val, index) => {
    if (!index && !uppercase) {
      res = `${res}${val}`;
    } else if (val.length) {
      res = `${res}${val.replace(/^[a-z]/i, val[0].toUpperCase())}`;
    }
  });
  return res;
}
...
function parseDateString(value) {
  ...
  const [, ymdYear, ymdMon, ymdDay, dmyDay, dmyMon, dmyYear] = match;

  const date = new Date();

  if (ymdYear !== undefined) {
    date.setFullYear(parseInt(ymdYear));
    date.setMonth(parseInt(ymdMon - 1));
    date.setDate(parseInt(ymdDay));
  } else {
    date.setFullYear(parseInt(dmyYear));
    date.setMonth(parseInt(dmyMon - 1));
    date.setDate(parseInt(dmyDay));
  }
...
function toDateString(value, format) {
  ...
  throw new Error('Invalid format string, expected `YYYY/MM/DD`, `DD/MM/YYYY`, or `MM/DD/YYYY`');
}
...
function normalizeTime(value) {
  try {
    let [, hour, min, sec, apm] = value.match(/(\d{1,2}):(\d{1,2}):(\d{1,2})( [AP]M)?/);
    ...
    return `(${hour}, ${parseInt(min)}, ${parseInt(sec)})`;
  } catch (error) {
    return null;
  }
}
...
function mimeTypeFromFilename(filename) {
  const [, fileExt] = filename.match(/(\.[a-z0-9]+)?$/i);
  switch (fileExt) {
    case '.txt':
      return 'text/plain';
    case '.html':
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  if ((resourceType === null) | (resourceType === undefined) | (resourceType === '')) {
    throw new Error(
      'Invalid argument provided. Variable resourceId must be provided and non-empty'
    );
  }
  ...
  if (extraData === '') {
    throw new Error('Invalid argument provided. Variable extraData must not be empty');
  }
  let apiEndPt = `/${resourceType}/${resourceId}`;
  if ((extraData === null) | (extraData === undefined)) {
    return apiEndPt;
  }
  return `${apiEndPt}/${encodeURIComponent(extraData)}`;
}

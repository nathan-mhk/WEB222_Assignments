...
function speciesCoordinates(data) {
  const { results: obsrvArr } = data;

  for (let i = 0; i < obsrvArr.length; ++i) {
    const obsrv = obsrvArr[i];
    console.log(`"${obsrv.species_guess}" observed at coordinates (${obsrv.location})`);
  }
}
...
function speciesCoordinates3(data) {
  ...
  for (const result of results) {
    console.log(
      `"${result.species_guess}" observed at coordinates (${result.location.replace(',', ', ')})`
    );
  }
}
...
function observationsByQualityGrade(data, qualityGrade) {
  if (qualityGrade === undefined || (typeof qualityGrade !== 'string' && qualityGrade !== null)) {
    throw new Error(
      'ERR Expected variable qualityGrade to be a string or null. Received undefined/unknown type'
    );
  }

  const grade = qualityGrade ? qualityGrade.toLowerCase() : null;
  ...
  switch (grade) {
    ...
    case null: {
      ...
      return results.filter((result) => result.quality_grade === grade);
    }
    default:
      throw new Error(
        `ERR Expected qualityGrade to be one of the following: "research", "needs_id", "casual", null. Received ${qualityGrade}`
      );
  }
}
...
function observationsByQualityGrades(data, ...qualityGrades) {
  if (qualityGrades === null || qualityGrades === undefined || qualityGrades.length < 1) {
    throw new Error('ERR Expected and array of string. Received null/undefined/unknown type.');
  }

  // Method 1
  // let res = [];
  // qualityGrades.forEach((grade) => (res = res.concat(observationsByQualityGrade(data, grade))));
  // return res;

  // Method 2
  return qualityGrades.flatMap((grade) => observationsByQualityGrade(data, grade));
}
...
function transformObservation(original) {
  ...
  //   isExtinct:
  //     original.conservation_status !== null &&
  //     original.conservation_status !== undefined &&
...
function transformObservations(data) {
  ...
  data.results.forEach((obsrvData) => res.push(transformObservation(obsrvData)));
...
function transformObservations2(data) {
  return data.results.map((obsvData) => transformObservation(obsvData));
}
...
function getObservationsByTaxa(data, ...taxaNames) {
  // Method 1
  const res = [];
  if (taxaNames !== null && taxaNames !== undefined && taxaNames.length) {
    taxaNames.forEach((taxon) => {
      switch (taxon) {
        ...
        case 'Plantae':
          res.push(
            ...data.results.filter((obsrvData) => obsrvData.taxon.iconic_taxon_name === taxon)
          );
          break;
        default:
          break;
  ...
  // Method 2
  // if (taxaNames !== null && taxaNames !== undefined && taxaNames.length) {
  //   return data.results.filter((obsrvData) =>
  //     taxaNames.some((taxon) => {
  //       switch (taxon) {
  //         case 'Animalia':
  //         case 'Aves':
  //         case 'Insecta':
  //         case 'Plantae':
  //           return obsrvData.taxon.iconic_taxon_name === taxon;
  //         default:
  //           return false;
  //       }
  //     })
...
function getObservationsByLocation(data, options = {}) {
  ...
  if (lat === undefined || lng === undefined) return data.results;
  ...
  if (typeof lat === 'object') {
    ({ min: latMin, max: latMax } = lat);
  }
  if (typeof lng === 'object') {
    ({ min: lngMin, max: lngMax } = lat);
  }

  return data.results.filter((obsrvData) => {
    const [locLat, locLng] = obsrvData.location.split(',').map((val) => parseFloat(val));
    ...
    if (latMin !== null) {
      // Range
      latMatch = locLat >= latMin && locLat <= latMax;
    } else {
      // Exact
      latMatch = locLat === lat;
    }
    ...
    if (lngMin !== null) {
      // Range
      lngMatch = locLat >= lngMin && locLng <= lngMax;
    } else {
      // Exact
      lngMatch = locLng === lng;
    }
...
function getPlaceURLs(data) {
  // Method 1
  // let res = [];
  // data.results.forEach((obsrvData) => {
  //   const { place_ids } = obsrvData;
  //   res = res.concat(
  //     place_ids.map((id) => `https://www.inaturalist.org/observations?place_id=${id}`)
  //   );
  // });
  // return res;

  // Method 2
  return data.results.flatMap((obsrvData) =>
    obsrvData.place_ids.map((id) => `https://www.inaturalist.org/observations?place_id=${id}`)
  );
}
...
function getSpeciesObservations(data) {
  ...
  data.results.forEach((obsrvData) => {
    const name = obsrvData.taxon.name;
    ...
    // res[name] = res[name] === undefined ? 1 : res[name] + 1;
  });
...
function extractSpeciesNames(data) {
  const res = [];
  data.results.map((obsrvData) => {
    const species = obsrvData.species_guess;
    if (res.indexOf(species) === -1) {
      res.push(species);
    }
  });
  return res;
...
function extractSpeciesNames2(data) {
  ...
  // data.results.map((obsrvData) => res.add(obsrvData.species_guess));
  // return Array.from(res);

  // Method 2
  return Array.from(new Set(data.results.flatMap((obsrvData) => obsrvData.species_guess)));
}
